...
module.exports = () => {
  ...
  function _iterableToArray(iter) {
    if ( Object.prototype.toString.call(iter) === "[object Arguments]") return Array.from(iter);
  }
  ...
  function domNodesToCdt(docNode) {
    ...
    function elementNodeFactory(domNodes, elementNode) {
      let node;
      const nodeType = elementNode.nodeType;
  ...
  function aggregateResourceUrlsAndBlobs(resourceUrlsAndBlobsArr) {
    return resourceUrlsAndBlobsArr.reduce((_ref, _ref2) => {
      let allResourceUrls = _ref.resourceUrls,
          allBlobsObj = _ref.blobsObj;
      let resourceUrls = _ref2.resourceUrls,
  ...
  function makeGetResourceUrlsAndBlobs(_ref) {
    let processResource = _ref.processResource,
      ...
      function doProcessResource(url) {
        return fetchUrl(url).catch(e => {
          ...
          if (/text\/css/.test(type)) {
            ...
            return getResourceUrlsAndBlobs(baseUrl, resourceUrls).then((_ref3) => {
              let resourceUrls = _ref3.resourceUrls,
  ...
  function fetchUrl(url) {
    let fetch = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : window.fetch;
    return fetch(url, {
      cache: 'force-cache',
  ...
  function makeFindStyleSheetByUrl(_ref) {
    let styleSheetCache = _ref.styleSheetCache;
  ...
  function isSameOrigin(url, baseUrl) {
    ...
    const _ref = new URL(url, baseUrl),
          origin = _ref.origin;

    const _ref2 = new URL(baseUrl),
          baseOrigin = _ref2.origin;
  ...
  function processPage() {
    ...
    function doProcessPage(doc) {
      ...
      return Promise.all([resourceUrlsAndBlobsPromise].concat(_toConsumableArray(processFramesPromise))).then((_ref) => {
  ...
  function serializeFrame(frame) {
    frame.blobs = frame.blobs.map((_ref) => {
      let url = _ref.url,
          type = _ref.type,
          value = _ref.value;
      return {

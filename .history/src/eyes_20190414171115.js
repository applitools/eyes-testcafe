...
const {processPageAndSerialize} = require('@applitools/dom-snapshot/src/browser/processPageAndSerialize');
...
class EyesBase {
  ...
  async eyesOpen(config) {
    this._processPage = ClientFunction(await getProcessPageScript(),  { dependencies: { getDocumentURI } });

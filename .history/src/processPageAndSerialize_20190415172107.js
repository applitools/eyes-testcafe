module.exports = () => {
  ...
  function extractLinks(doc = document) {
    const srcsetUrls = _toConsumableArray(doc.querySelectorAll('img[srcset],source[srcset]'))
      .map(srcsetEl =>
        srcsetEl
          .getAttribute('srcset')
          .split(',')
    ...
    const srcUrls = _toConsumableArray(doc.querySelectorAll('img[src],source[src]')).map(srcEl =>
      srcEl.getAttribute('src'),
    );

    const cssUrls = _toConsumableArray(doc.querySelectorAll('link[rel="stylesheet"]')).map(link =>
      link.getAttribute('href'),
    );

    const videoPosterUrls = _toConsumableArray(doc.querySelectorAll('video[poster]')).map(videoEl =>
      videoEl.getAttribute('poster'),
    );

    return [].concat(
      _toConsumableArray(srcsetUrls),
      _toConsumableArray(srcUrls),
      _toConsumableArray(cssUrls),
      _toConsumableArray(videoPosterUrls),
    );
  }
          ...
          if (
            ...
            elementNode.appendChild(
              docNode.createTextNode(
                _toConsumableArray(elementNode.sheet.cssRules)
  ...
  function extractFrames(doc = document) {
    return _toConsumableArray(doc.querySelectorAll('iframe[src]:not([src=""])'))
      .map(srcEl => {
        try {
  ...
  function makeGetResourceUrlsAndBlobs({processResource, aggregateResourceUrlsAndBlobs}) {
    return function getResourceUrlsAndBlobs(doc, baseUrl, urls) {
      return Promise.all(
        urls.map(url =>
          processResource(url, doc, baseUrl, getResourceUrlsAndBlobs.bind(null, doc)),
        ),
  ...
  function makeFindStyleSheetByUrl({styleSheetCache}) {
    return function findStyleSheetByUrl(url, doc) {
      return (
        styleSheetCache[url] ||
        _toConsumableArray(doc.styleSheets).find(styleSheet => styleSheet.href === url)
      );
    };
  }
  ...
  function makeExtractResourcesFromStyleSheet({styleSheetCache}) {
    return function extractResourcesFromStyleSheet(styleSheet, win = window) {
      return uniq_1(
        _toConsumableArray(styleSheet.cssRules || []).reduce((acc, rule) => {
  ...
  function makeExtractResourceUrlsFromStyleTags(extractResourcesFromStyleSheet) {
    return function extractResourceUrlsFromStyleTags(doc) {
      return uniq_1(
        _toConsumableArray(doc.getElementsByTagName('style')).reduce((resourceUrls, styleEl) => {
          const styleSheet = _toConsumableArray(doc.styleSheets).find(
  ...
  function processPage(doc = document) {
    ...
    function doProcessPage(doc) {
      ...
      return Promise.all(
        [resourceUrlsAndBlobsPromise].concat(_toConsumableArray(processFramesPromise)),
  ...
  module.exports = processPageAndSerialize_1;
